...
export async function ensureMcpApi(): Promise<void> {
...
export function stopManagedMcpApi() {
  if (managedApi?.exitCode === null) {
    managedApi.kill('SIGTERM')
  }
}

process.once('SIGINT', stopManagedMcpApi)
process.once('SIGTERM', stopManagedMcpApi)
process.once('exit', stopManagedMcpApi)
...
export function createVocabularyMcpServer(): McpServer {
  ...
  MCP_SERVER.registerTool(
    ...
    async ({ senseId }) => {
      ...
      return result(
        data,
        `Loaded sense ${senseId} for ${String(data.word ?? 'the requested word')}.`,
      )
    },
  )
  ...
  MCP_SERVER.registerTool(
    'set_cefr_level',
    {
      ...
      description:
        'Set the A1-C2 level for a specific sense after confirming the intended meaning.',
      inputSchema: {
        senseId: z.number().int().positive(),
...
const IS_STDIO_ENTRY = path.resolve(process.argv[1] ?? '') === fileURLToPath(import.meta.url)

if (IS_STDIO_ENTRY) {
  await ensureMcpApi()
  await createVocabularyMcpServer().connect(new StdioServerTransport())
}
